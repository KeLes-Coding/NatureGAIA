...
def generate_api(images, query):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(process_image, image, query): i
...
def ocr_parallel(
    img, ocr_detection, ocr_recognition, img_x_list, img_y_list, padding, i
):
...
def get_perception_infos(screenshot_file, screenshot_som_file, font_path):
    ...
    img_list = [
        "./screenshot/screenshot_part_1.png",
        "./screenshot/screenshot_part_2.png",
        "./screenshot/screenshot_part_3.png",
        "./screenshot/screenshot_part_4.png",
    ]
...
ocr_recognition = pipeline(
    Tasks.ocr_recognition, model="damo/cv_convnextTiny_ocr-recognition-document_damo"
)
...
while True:
    ...
    if args.use_som == 1:
        chat_action = add_response(
            "user", prompt_action, chat_action, [screenshot_file, screenshot_som_file]
        )
    else:
        chat_action = add_response(
